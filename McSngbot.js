...
//version 1.1.4
    ...
    const autoeat = require("mineflayer-auto-eat") //讀取mineflayer-auto-eat模塊
...
//取得時間
    ...
    function connects() {
        tokens.use(loginOpts, function (_err, _opts) { //使用驗證緩存
            ...
            bot.once('spawn', () => {   //bot啟動時
                console.log(`【McSngbot】 載入完成`)
                if (settings.AutoEat) { //如果自動飲食設定為True時，則執行(預設為True)
                    require("./Utility/AutoEat")(bot, autoeat);
                }
                //自動宣傳
    ...
    console.log("【MCSngBot】發生意外錯誤，請至我們的Discord伺服器詢問")
}